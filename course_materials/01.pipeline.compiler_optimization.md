# Pipeline Examples and Compiler Optimizations

Learning goals:
  * Understand the processor pipeline
    * stages of execution
    * overlapped/parallel instructions
  * Understand pipeline stalls
    * RAW (read after write) conflicts
  * Be able to reproduce RAW hazards and mitigate with a variety of techniques:
    * independent chains
    * multiple accumulators
    * common subexpression elimination
    * dead code elimination
    * temporary variables (in activity 1)
  * Understand the limitations of the compiler
    * analyze what optimizations the compiler is making for you
    * identify cases where the compiler can't optimize and do it by hand
  * Examine complier out-of-order execution in a symbolic debugger

# Pipeline Examples

   [examples/pipeline/README.pipeline.md](../examples/pipeline/README.pipeline.md)
...
*RB* Let's go debug `pipeline.cpp` to see the effects of optimization.
  * show literal order at -O0
  * demonstrate out of order execution at -O2
  * examine disassembly to verify it an OOO static schedule
 
## RB Finish 01 (09/10)

  * go back to independent chains. show the code of using different accumulators at each line.
    * compare the -O0 and -O1 results (see pipeline.nochain.md)
  * run both through debugger to show -O0 is literal generated code
  * revisit the difference between independent chains and multiple accumulators 
    * (reanswer question) [Two Ways to Break a Chain](../examples/pipeline/pipeline.chains.html)
  * do the next two examples: dead code and common sub-expression.
  * then on to speedup

## Why we rely on `-O1` for many examples?

`-O0` hides real effects behind stack-traffic noise, and `-O2`/`-O3` often 
*discover the same optimization the example is trying to teach you*. 
`-O1` is the level where the compiler has cleaned up the accidental slowness of `-O0` but hasn't yet
automated the specific trick a given example. We are most interested in examples that the compiler doesn't find.
This is when programming matters.  We'll see many of our examples run well at `-02` and `-O3` also if you turn off the auto-vectorizer.

[pipeline/README.pipeline.md](../examples/pipeline/README.pipeline.md) makes this explicit for
