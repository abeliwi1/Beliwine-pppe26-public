"""
forkjoinsum_chart.py

Runs `java ForkJoinSum.java` at varying pool sizes, then plots:
...
def run_forkjoinsum(n_threads):
    result = subprocess.run(
        ["java", "ForkJoinSum.java", str(N), str(n_threads)],
        capture_output=True, text=True
    )
    times = re.findall(r"forkjoin\s+sum=\d+\s+time=([0-9.]+)", result.stdout)
    times = [float(t) for t in times]
    # first call after warm-up can still be JIT-cold; take median of all trials
    return np.median(times)


print("Running ForkJoinSum benchmarks...")
times = {}
for t in THREAD_COUNTS:
    times[t] = run_forkjoinsum(t)
...
fig.suptitle(f"ForkJoinSum Scaling  (N={N:,})", fontsize=13)
